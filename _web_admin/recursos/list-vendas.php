    ...
    <table name="itemsOrigem" id="itemsOrigem" multiple="multiple" class="tabelaCarrinho" cellpadding="0" cellspacing="0">
	   ...
	   <tbody>
        <?php
			...
			$banco = new Banco();
         $sql_visualizar = $banco->query("SELECT cd_venda, nm_cliente, dt_pagamento,status, valor  from venda inner join cliente on(cliente.cd_cliente = venda.Cliente_cd_cliente)");
